...
import { chapters, resources, subjects } from '@/data/mockData';
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <Card className="mb-12">
            ...
            <CardContent className="space-y-6">
              {Object.keys(resourceSets).length > 0 ? (Object.entries(resourceSets).map(([setId, set]) => {
            const questionPaper = set.resources.find(r => r.type === 'question_paper');
            const answerKey = set.resources.find(r => r.type === 'answer_key');
            return (<div key={setId} className="p-6 border-2 rounded-lg bg-gradient-to-br from-muted/30 to-muted/10">
                      ...
                      <div className="grid md:grid-cols-2 gap-4 mt-4">
                        {questionPaper && (<div className="p-4 bg-background border rounded-lg">
                        ...
                        {answerKey && (<div className="p-4 bg-background border rounded-lg">
                            <div className="flex items-center gap-2 mb-2">
            ...
            <CardContent className="space-y-6">
              ...
              <form onSubmit={handleSubmitComment} className="space-y-4 p-4 bg-muted/30 rounded-lg">
                <div className="grid md:grid-cols-2 gap-4">
                  <Input placeholder="Your name *" value={newComment.name} onChange={(e) => setNewComment({ ...newComment, name: e.target.value })}/>
                  <Input type="email" placeholder="Your email (optional)" value={newComment.email} onChange={(e) => setNewComment({ ...newComment, email: e.target.value })}/>
                </div>
                <Textarea placeholder="Write your comment... *" value={newComment.text} onChange={(e) => setNewComment({ ...newComment, text: e.target.value })} rows={4}/>
                <Button type="submit">Post Comment</Button>
              </form>

              {/* Comments List */}
              <div className="space-y-4">
                {comments.length > 0 ? (comments.map((comment) => (<div key={comment.id} className="p-4 border rounded-lg">
